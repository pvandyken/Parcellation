...
map<int, float> BundleIntersections::getTriangleProbabilities(
    int bundleId, Mesh &mesh, const vector<map<int, int>> &triangleIntersections
) {
    map<int, float> probabilities;
    for (auto const& triangle : this->triangles[bundleId]) {
        ...
        for (auto const& neighbor : neighbors) {
            ...
            const int &bundleTotal = (
                neighborIntersections.find(bundleId) != neighborIntersections.end()
             ) ? neighborIntersections.at(bundleId) : 0;
