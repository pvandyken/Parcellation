...
void IO::write_intersection(
    const string &path, const string &pathBundles,
  ...
  for (uint16_t i = 0; i < fib_index.size(); i++) {  // 50
    ...
    if (file.is_open()) {
      ...
      file << "\n";
      for (auto triangle :
           frontIntersections[i]
               .triangles)  // writes index of each initial triangle
        file << triangle << " ";
      file << "\n";
      for (auto triangle :
           backIntersections[i]
               .triangles)  // writes index of each final triangle
        file << triangle << " ";
      file << "\n";
      for (auto point :
           frontIntersections[i]
               .points) {  // writes the initial intersection point (x,y,z)
        for (auto coord : point) file << coord << " ";
      }
      file << "\n";

      for (auto point :
           backIntersections[i]
               .points) {  // writes the final intersection point (x,y,z)
        for (auto coord : point) file << coord << " ";
      }
      file << "\n";
