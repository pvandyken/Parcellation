...
void Orientation::alignOrientation(vector<vector<Vector3f>> &fibers) {
  ...
  // Use first fiber as our reference. It's aligned by definition, so we add
  // true as the first entry to directions
  const vector<Vector3f> &refFiber = fibers[0];
  directions.push_back(true);

  // Loop through all other fibers and check if they're aligned to the
  // reference. Store the results in the directions array. Keep track of the
  // number of aligned and misaligned fibers
  int alignments = 0;
  int misalignments = 0;
...
bool Orientation::isAligned(const vector<Vector3f> &fiber,
